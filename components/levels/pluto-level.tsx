...
import { ArrowLeft, Volume2, ArrowRight, RotateCcw, CheckCircle2, XCircle } from "lucide-react"
...
const question = {
  title: "Pluto's Size",
  text: "Is Pluto Big or Small?",
  soundFile: "/soundQuetion/9.mp3", // 🔊 ملف الصوت المطلوب
  options: [
    { id: "big", label: "Big", emoji: "❄️", sizeClass: "text-8xl", isCorrect: false },
    { id: "small", label: "Small", emoji: "❄️", sizeClass: "text-5xl", isCorrect: true },
  ],
}
...
export default function PlutoLevel({ onComplete, onBack }: PlutoLevelProps) {
  ...
  const handleMainButton = () => {
    ...
    if (selectedOption?.isCorrect) {
      ...
      setFeedbackMessage("Oops! Pluto is actually very small (Dwarf Planet).")
    }
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl flex flex-col items-center z-10">
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="mb-8 flex flex-col items-center">
          <div className="mb-4 text-8xl inline-block animate-bounce">
            🥶
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-blue-300 mb-6 drop-shadow-lg">{question.title}</h2>
          ...
          {question.options.map((option, index) => {
            ...
            return (
              ...
              >
                ...
                >
                    {/* تحكم في حجم الأيقونة بناءً على الخيار */}
                    <div className={`${option.sizeClass} drop-shadow-2xl transition-transform group-hover:rotate-12`}>
                        {option.emoji}
                    </div>
          ...
          >
            <div className="text-center p-8">
              ...
              <motion.h3
                ...
                animate={{ y: 0, opacity: 1 }}
                className="text-6xl font-bold text-blue-300 mb-2"
              >
                Correct! 🎉
              </motion.h3>
