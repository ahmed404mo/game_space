...
const question = {
  ...
  text: "Can you find ALL the continents on Earth?",
  options: [
    { id: "africa", label: "Africa", emoji: "🦁", isCorrect: true },
    { id: "asia", label: "Asia", emoji: "🐼", isCorrect: true },
    { id: "europe", label: "Europe", emoji: "🏰", isCorrect: true },
    { id: "antarctica", label: "Antarctica", emoji: "🐧", isCorrect: true },
    { id: "mars", label: "Mars", emoji: "🔴", isCorrect: false },
    { id: "jupiter", label: "Jupiter", emoji: "🪐", isCorrect: false },
  ],
}

export default function EarthLevel({ onComplete, onBack }: LevelProps) {
  ...
  const playQuestionSound = () => {
    try {
      const audio = new Audio("/soundQuetion/3.mp3")
      audio.play().catch(() => {})
    } catch (e) {
  ...
  const handleMainButton = () => {
    // 1. إعادة المحاولة: مسح كل الاختيارات
    if (status === "wrong") {
      setStatus("playing")
      setFeedbackMessage("")
      setSelectedOptions([]) 
    ...
    // 2. التحقق من الإجابة
    const correctOptionsIds = question.options.filter((o) => o.isCorrect).map((o) => o.id)
    const selectedCorrect = selectedOptions.filter((id) => correctOptionsIds.includes(id))
    const selectedWrong = selectedOptions.filter((id) => !correctOptionsIds.includes(id))

    const isMissingSome = selectedCorrect.length < correctOptionsIds.length
    const hasWrongSelection = selectedWrong.length > 0

    if (!hasWrongSelection && !isMissingSome) {
      // ✅ إجابة صحيحة
      setStatus("correct")
      ...
      if (hasWrongSelection) {
        setFeedbackMessage("Oops! Mars and Jupiter are planets!")
      } else if (isMissingSome) {
        setFeedbackMessage("You missed some continents!")
      }
    }
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl flex flex-col items-center z-10">
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="mb-8 flex flex-col items-center">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-blue-400 mb-6 drop-shadow-lg">{question.title}</h2>
          
          {/* 🔥 تصميم السؤال: شريط أبيض + نص أسود 🔥 */}
          <div className="flex items-center gap-4 bg-white px-8 py-4 rounded-full shadow-2xl transform hover:scale-105 transition-transform duration-300 border-4 border-blue-200">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 mb-8 w-full max-w-4xl">
          {question.options.map((option, index) => {
            ...
            if (status === "playing") {
                ...
                if (option.isCorrect) { borderColor = "border-green-500"; bgColor = "bg-green-500/40" }
            } else if (status === "wrong") {
                if (isSelected && !option.isCorrect) { borderColor = "border-red-500"; bgColor = "bg-red-500/40" }
                else if (isSelected && option.isCorrect) { borderColor = "border-green-500"; bgColor = "bg-green-500/20" }
            }
            ...
            return (
              ...
              >
                <Button
                    ...
                    className={`h-32 w-full flex-col gap-2 rounded-3xl p-4 transition-all hover:scale-105 border-4 ${borderColor} ${bgColor}`}
                >
                    <span className="text-5xl drop-shadow-md">{option.emoji}</span>
                    <span className="text-lg font-bold text-white tracking-wide">{option.label}</span>
        ...
        <div className="mb-8">
          <Button
            ...
            disabled={selectedOptions.length === 0 && status === "playing"}
